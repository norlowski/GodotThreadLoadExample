...
class Log:
	...
	var prev_ticks_msec = 0.0
	
	# LOGGING 
	const LOG_FORMAT = "[{current_time}] | [{elapsed_time}] | {level} | [{script_name}] [{function_name}] >> {msg}"
	...
	func _log(level, message, function_name = ""):
		...
		var log_message = LOG_FORMAT.format({"level": level, "current_time": _get_current_time(),"elapsed_time":_get_elapsed_time() ,"script_name": script_name, "function_name": function_name, "msg": message})
		#print(log_message)
		file_writer.write(log_message)
		
	func _get_elapsed_time():
#		var elapsed_seconds_format = "{seconds}"
		var ts_diff = OS.get_ticks_msec() - prev_ticks_msec
		prev_ticks_msec = OS.get_ticks_msec()
		return str(ts_diff/1000.0)
